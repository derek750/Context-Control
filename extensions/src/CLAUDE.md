...
- **`extension.ts`**: registers commands, wires configuration, creates the panel, connects the WS bridge. Before `ProxyManager.start`, calls `ensureReady` from **`python-bootstrap.ts`** (unless bootstrap fails — then no proxy start). **`autonomy.retryPythonSetup`** calls `ensureReady(..., force=true)`.
- **`python-bootstrap.ts`**: `ensureReady` — if `autonomy.pythonPath` is set, validate path + min version (3.10) and return it; else discover base Python, create/reuse venv under `context.globalStorageUri/autonomy-venv/<hash>`, run `pip install -r <backendDir>/requirements.txt`, cache by requirements SHA-256 in `globalState`.
